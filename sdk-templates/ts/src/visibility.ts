...
export function distance(a: Position, b: Position): number {
  if (a.room !== b.room) return Number.POSITIVE_INFINITY;
  return Math.max(Math.abs(a.x - b.x), Math.abs(a.y - b.y), Math.abs(a.x + a.y - b.x - b.y));
}

export function visionRange(entity: WorldEntity): number {
  if (typeof entity.vision_range === "number") return entity.vision_range;
  if (entity.type === "drone") return 3;
  if (entity.type === "structure" && "structure_type" in entity) {
    if (entity.structure_type === "Observer") return 10;
    if (entity.structure_type === "Tower") return 3;
    if (entity.structure_type === "Spawn") return 3;
  }
  if (entity.type === "controller" && entity.owner) return 1;
  return 0;
}

export function isVisibleTo(entity: WorldEntity, context: VisibilityContext): boolean {
  if (context.fog_of_war === false) return true;
  if (entity.owner === context.player_id) return true;
...
export function visibleEntities(entities: WorldEntity[], player_id: PlayerId, tick: number | bigint, fog_of_war = true): WorldEntity[] {
...
export function snapshotUsesFogOfWar(config: WorldConfig): boolean {
