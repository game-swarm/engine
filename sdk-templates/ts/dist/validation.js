import { ABI_VERSION, MAX_BODY_PARTS, MAX_COMMANDS_PER_PLAYER, MAX_JSON_DEPTH, MAX_TICK_OUTPUT_BYTES, } from "./commands.js";
...
const bodyParts = new Set(["Move", "Work", "Carry", "Attack", "RangedAttack", "Heal", "Claim", "Tough"]);
const damageTypes = new Set(["Kinetic", "Thermal", "EMP", "Sonic", "Corrosive", "Psionic"]);
...
export function encodeTickInput(input) {
...
export function decodeTickInput(input) {
    if (!(input instanceof Uint8Array))
        return fail("$", "tick input must be binary ABI v2 bytes", "BinaryRequired");
    if (input.byteLength > MAX_TICK_OUTPUT_BYTES)
        return fail("$", "tick input exceeds 256KB", "MaxBytes");
    try {
        ...
        reader.finish();
        if (!bytesEqual(encodeTickInput(result), input))
            throw new Error("tick input is not canonical ABI v2");
...
export function encodeTickResult(result) {
...
export function decodeTickResult(input) {
    if (!(input instanceof Uint8Array))
        return fail("$", "tick output must be binary ABI v2 bytes", "BinaryRequired");
    if (input.byteLength > MAX_TICK_OUTPUT_BYTES)
        return fail("$", "tick output exceeds 256KB", "MaxBytes");
    try {
        ...
        reader.finish();
        const result = { commands, messages };
        const validation = validateCommandIntents(commands);
        if (!validation.ok)
            return { ok: false, issues: validation.issues };
        if (!bytesEqual(encodeTickResult(result), input))
            throw new Error("tick output is not canonical ABI v2");
        return { ok: true, value: result, issues: [] };
    }
    catch (error) {
    ...
    switch (typeof value) {
        case "bigint":
            if (value < 0n || value > MAX_UINT64)
                throw new RangeError("bigint value is outside the u64 range");
...
function encodeCommandIntent(writer, command) {
...
function decodeCommandIntent(reader) {
...
function encodePlayerMessage(writer, message) {
    if (message.channel !== "Player" && message.channel !== "Debug")
        throw new Error("unknown PlayerMessage channel");
    if (typeof message.text !== "string")
        throw new Error("PlayerMessage text must be a string");
    writer.u32(message.channel === "Player" ? 0 : 1);
    writer.string(message.text);
}
function decodePlayerMessage(reader) {
    const channelTag = reader.u32();
    if (channelTag !== 0 && channelTag !== 1)
        throw new Error(`unknown MessageChannel discriminant ${channelTag}`);
...
function encodeAction(writer, action) {
    switch (action.type) {
        case "Move":
            writer.u32(1);
            writer.u64(action.object_id);
            writer.u32(directionTag(action.direction));
            return;
        case "Harvest":
            writer.u32(2);
            writer.u64(action.object_id);
            writer.u64(action.target_id);
            writer.option(action.resource, (value) => writer.string(value));
            return;
        case "Transfer":
            writer.u32(3);
            writer.u64(action.object_id);
            writer.u64(action.target_id);
            writer.string(action.resource);
            writer.u32(action.amount);
            return;
        case "Withdraw":
            writer.u32(4);
            writer.u64(action.object_id);
        ...
        case "Spawn":
            writer.u32(7);
            writer.u64(action.object_id);
            writer.u64(action.spawn_id);
            writer.vector(action.body_parts, (part) => writer.u32(bodyPartTag(part)));
            return;
        case "Recycle":
            writer.u32(8);
            writer.u64(action.object_id);
        ...
        case "Build":
            writer.u32(9);
            writer.u64(action.object_id);
            writer.string(action.structure);
            writer.i32(action.x);
            writer.i32(action.y);
            return;
        case "Repair":
        ...
        case "AlliedTransfer":
            writer.u32(14);
            writer.u32(action.target_player);
            writer.string(action.resource);
            writer.u32(action.amount);
            return;
        default:
...
function encodeGenericAction(writer, action) {
    const record = action;
    writer.u32(5);
    writer.string(action.type);
    writer.u64(record.object_id);
...
function decodeAction(reader) {
    ...
    switch (tag) {
        ...
        case 2: {
            ...
            const target_id = reader.u64();
            const resource = reader.option(() => reader.string());
        ...
        case 9: return { type: "Build", object_id: reader.u64(), structure: reader.string(), x: reader.i32(), y: reader.i32() };
...
function decodeGenericAction(reader) {
    ...
    let payload;
    try {
        payload = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(payloadBytes), (_key, value, context) => {
            if (typeof value === "number" && context?.source && /^\d+$/.test(context.source)) {
                const exact = BigInt(context.source);
                if (exact > BigInt(Number.MAX_SAFE_INTEGER) && exact <= MAX_UINT64)
                    return exact;
            }
            return value;
        });
        ...
        throw new Error("invalid generic Action payload");
    }
    if (!isRecord(payload))
        throw new Error("generic Action payload must be an object");
    return { type, object_id, ...payload };
}
...
const bodyPartTags = { Move: 1, Work: 2, Carry: 3, Attack: 4, RangedAttack: 5, Heal: 6, Claim: 7, Tough: 8 };
const bodyPartsByTag = ["Move", "Work", "Carry", "Attack", "RangedAttack", "Heal", "Claim", "Tough"];
function bodyPartTag(part) { return bodyPartTags[part]; }
function bodyPartFromTag(tag) {
    const part = bodyPartsByTag[tag - 1];
    if (!part)
        throw new Error(`unknown BodyPart discriminant ${tag}`);
    return part;
}
class BinaryWriter {
    output = [];
    u32(value) { this.number(value, 4, false); }
    i32(value) { this.number(value, 4, true); }
    u64(value) {
        const exact = typeof value === "bigint" ? value : BigInt(value);
        if (exact < 0n || exact > MAX_UINT64)
            throw new Error("u64 value is out of range");
        for (let shift = 0n; shift < 64n; shift += 8n)
            this.output.push(Number((exact >> shift) & 0xffn));
    }
    string(value) { this.bytes(new TextEncoder().encode(value)); }
    bytes(value) { this.u32(value.byteLength); this.raw(value); }
    raw(value) { this.output.push(...value); }
    option(value, encode) { this.output.push(value === undefined ? 0 : 1); if (value !== undefined)
        encode(value); }
    vector(values, encode) { this.u32(values.length); values.forEach(encode); }
    finish() { return Uint8Array.from(this.output); }
    number(value, size, signed) {
        if (!Number.isInteger(value))
            throw new Error("integer value required");
        const bytes = new Uint8Array(size);
        new DataView(bytes.buffer).setInt32(0, value, true);
        if (!signed && value < 0)
            throw new Error("unsigned integer value required");
        this.raw(bytes);
    }
}
class BinaryReader {
    ...
    u32() { return this.number(false); }
    i32() { return this.number(true); }
    u64() {
    ...
    bytes() { return this.raw(this.u32()); }
    raw(length) { if (length < 0 || this.offset + length > this.input.byteLength)
        throw new Error("unexpected end of ABI payload"); const value = this.input.slice(this.offset, this.offset + length); this.offset += length; return value; }
    option(decode) { const tag = this.raw(1)[0]; if (tag === 0)
        return undefined; if (tag !== 1)
        throw new Error(`invalid option tag ${tag}`); return decode(); }
    vector(decode, max = 16_384) { const length = this.u32(); if (length > max)
        throw new Error(`vector length ${length} exceeds ${max}`); return Array.from({ length }, decode); }
    expectU32(expected, field) { const actual = this.u32(); if (actual !== expected)
        throw new Error(`${field} mismatch: expected ${expected}, got ${actual}`); }
    finish() { if (this.offset !== this.input.byteLength)
        throw new Error("trailing bytes in ABI payload"); }
    number(signed) { const bytes = this.raw(4); return signed ? new DataView(bytes.buffer, bytes.byteOffset, 4).getInt32(0, true) : new DataView(bytes.buffer, bytes.byteOffset, 4).getUint32(0, true); }
}
function bytesEqual(left, right) {
    return left.byteLength === right.byteLength && left.every((byte, index) => byte === right[index]);
}
function encodeEnvelope(messageTag, payload, field) {
    ...
    if (output.byteLength > MAX_TICK_OUTPUT_BYTES)
        throw new Error(`Invalid ${field}: exceeds 256KB`);
    return output;
}
function decodeEnvelope(input, messageTag, messageName) {
    ...
    if (payloadLength !== input.byteLength - HEADER_BYTES)
        throw new Error(`${messageName} payload length mismatch`);
    return reader;
}
export function validateCommandIntents(value) {
    const issues = [];
    if (!Array.isArray(value))
        return fail("$", "tick output must be an array", "ArrayRequired");
    if (value.length > MAX_COMMANDS_PER_PLAYER)
        issue(issues, "$", `more than ${MAX_COMMANDS_PER_PLAYER} commands`, "MaxItems");
    if (jsonDepth(value) > MAX_JSON_DEPTH)
        issue(issues, "$", `JSON depth exceeds ${MAX_JSON_DEPTH}`, "MaxDepth");
    value.forEach((item, index) => validateCommandIntent(item, `$[${index}]`, issues));
    if (issues.length > 0)
        return { ok: false, issues };
    return { ok: true, value: value, issues };
}
export function validateCommandIntent(value, path = "$", issues = []) {
    if (!isRecord(value)) {
...
export function validateAction(value, path = "$", issues = []) {
    if (!isRecord(value)) {
    ...
    switch (value.type) {
        ...
        case "Spawn":
            ...
            if (!Array.isArray(value.body_parts))
                issue(issues, `${path}.body_parts`, "body_parts must be an array", "ArrayRequired");
            else {
                if (value.body_parts.length === 0)
                    issue(issues, `${path}.body_parts`, "body_parts must not be empty", "BodyEmpty");
                if (value.body_parts.length > MAX_BODY_PARTS)
                    issue(issues, `${path}.body_parts`, `body_parts exceeds ${MAX_BODY_PARTS} parts`, "BodyTooLarge");
        ...
        case "Leech":
            ...
            if (value.cooldown !== undefined)
                u32(value.cooldown, `${path}.cooldown`, issues);
            break;
        case "Fortify":
        case "Fabricate":
            objectId(value.object_id, `${path}.object_id`, issues);
...
function exactKeys(value, allowed, path, issues) {
    for (const key of Object.keys(value)) {
        if (!allowed.includes(key))
            issue(issues, `${path}.${key}`, "field is not allowed", "AdditionalProperty");
    }
    for (const key of allowed) {
        if (key !== "resource" && key !== "target_id" && key !== "damage_type" && key !== "cooldown" && key !== "client_trace_id" && !(key in value))
            issue(issues, `${path}.${key}`, "field is required", "Required");
    }
}
function isRecord(value) {
    return typeof value === "object" && value !== null && !Array.isArray(value);
}
function u32(value, path, issues) {
    if (!Number.isInteger(value) || value < 0 || value > 0xffffffff)
        issue(issues, path, "must be a u32", "InvalidU32");
}
function objectId(value, path, issues) {
    ...
    if (!validNumber && !validBigint)
        issue(issues, path, "must be a u64 ObjectId", "InvalidObjectId");
}
function coord(value, path, issues) {
    if (!Number.isInteger(value) || value < MIN_ROOM_COORD || value > MAX_ROOM_COORD) {
        issue(issues, path, `must be an i32 room coordinate in [${MIN_ROOM_COORD}, ${MAX_ROOM_COORD}]`, "InvalidCoordinate");
    }
}
function boundedString(value, path, issues) {
    if (typeof value !== "string" || value.length === 0 || value.length > MAX_STRING_LENGTH)
        issue(issues, path, "must be a non-empty bounded string", "InvalidString");
}
function enumValue(value, allowed, path, issues) {
    if (typeof value !== "string" || !allowed.has(value))
        issue(issues, path, "invalid enum value", "InvalidEnum");
}
function issue(issues, path, message, code) {
