...
export function createDefaultWorldConfig(mode = "persistent") {
...
export function validateWorldConfig(config) {
    const issues = [];
    if (config.world.tick_interval_ms < 1000)
        push(issues, "world.tick_interval_ms", "tick interval must be at least 1000ms", "TickIntervalTooShort");
    if (config.code.propagation_speed < 0 || config.code.propagation_speed > 100)
        push(issues, "code.propagation_speed", "propagation speed must be 0..100", "InvalidPropagationSpeed");
    if (config.drone.memory_size > 65536)
        push(issues, "drone.memory_size", "memory size must not exceed 64KB", "MemoryTooLarge");
    if (config.drone.max_body_parts > MAX_BODY_PARTS)
        push(issues, "drone.max_body_parts", "max body parts must not exceed 50", "BodyTooLarge");
    if (config.combat.damage_multiplier <= 0)
        push(issues, "combat.damage_multiplier", "damage multiplier must be positive", "InvalidDamageMultiplier");
    ...
    for (const [path, cost] of Object.entries(config.actions.costs)) {
        for (const resource of Object.keys(cost)) {
            if (!resourceNames.has(resource))
                push(issues, `actions.costs.${path}.${resource}`, "resource is not declared", "UnknownResource");
        }
    }
    return issues;
}
export function actionCost(config, action, detail) {
    const key = detail ? `${action}.${detail}` : action;
    return { ...(config.actions.costs[key] ?? {}) };
}
export function bodyPartCost(config, part) {
    return actionCost(config, "body_part", part);
}
export function bodyCostFromRules(config, body) {
    const total = {};
    for (const part of body)
        addCost(total, bodyPartCost(config, part));
    return total;
}
function push(issues, path, message, code) {
    issues.push({ path, message, code });
}
//# sourceMappingURL=worldRules.js.map